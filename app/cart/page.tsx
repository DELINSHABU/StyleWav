      ...
      {items.length === 0 ? (
        ...
        <div className="grid md:grid-cols-[2fr_1fr] gap-8 mt-6">
          <div className="space-y-4">
            {items.map((it, index) => (
              <div key={`${it.id}-${it.size || 'no-size'}-${index}`} className="flex items-center gap-4 border border-border rounded-lg p-3">
                <img
                ...
                <div className="flex-1">
                  ...
                  {it.size && (
                    <p className="text-sm text-muted-foreground font-medium">Size: {it.size}</p>
                  )}
                  ...
                  <div className="mt-2 inline-flex items-center gap-2">
                    <button
                      ...
                      aria-label={`Decrease ${it.name}${it.size ? ` (${it.size})` : ''}`}
                    ...
                    <button
                      className="h-7 w-7 rounded-md border border-border hover:bg-muted transition-colors"
                      aria-label={`Increase ${it.name}${it.size ? ` (${it.size})` : ''}`}
          ...
          <aside className="border border-border rounded-lg p-4 h-fit">
            ...
            <div className="mt-3 space-y-2">
              {items.map((item, index) => (
                <div key={`${item.id}-${item.size || 'no-size'}-${index}`} className="flex justify-between text-xs">
